...
import { GET_MESSAGES, SEND_MESSAGE, EDIT_MESSAGE, DELETE_MESSAGE, MARK_READ } from "@/lib/gql";
...
export function useSendMessage(roomId: string) {
  ...
  return useMutation({
    ...
    onMutate: async (data) => {
      ...
      queryClient.setQueryData<InfiniteData<Message[]>>(messageKeys.list(roomId), (old) => {
        if (!old) return old;
    ...
    onError: (err, data, context) => {
      ...
      queryClient.setQueryData<InfiniteData<Message[]>>(messageKeys.list(roomId), (old) => {
        ...
        return {
          ...old,
          pages: old.pages.map((page) => page.filter((m) => m.id !== context?.optimisticMsg.id)),
        };
      });
    },
      ...
      queryClient.setQueryData<InfiniteData<Message[]>>(messageKeys.list(roomId), (old) => {
        ...
        return {
          ...
          pages: old.pages.map((page) =>
            page.map((m) => (m.id === context?.optimisticMsg.id ? realMessage : m)),
          ),
        };
      });
    },
    ...
    onSuccess: (result) => {
      queryClient.setQueryData<InfiniteData<Message[]>>(messageKeys.list(roomId), (old) => {
        ...
        return {
          ...old,
          pages: old.pages.map((page) =>
            page.map((m) => (m.id === result.id ? { ...m, is_deleted: true, content: "" } : m)),
          ),
        };
      });
    },
...
export function useMarkRead() {
  return useMutation({
    mutationFn: (data: { room_id: string; user_id: string; message_id: string }) =>
      gqlClient
        .request<{ markRead: MessageRead }>(MARK_READ, { input: data })
        .then((d) => d.markRead),
  });
}
