...
export function Avatar({username, avatarUrl, isOnline,size='md'} : AvatarProps) {
    return (
        <div className="relative flex shrink-0">
            {avatarUrl ? (
                <img src={avatarUrl} alt={username} className={cn("rounded-full object-cover",
                    sizes[size]
                )} />
            ) : (
                <div
                    className={cn("rounded-full flex items-center justify-center font-semibold text-white select-none",
                        sizes[size],
