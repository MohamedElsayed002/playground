...
export function RealtimeChatPanel() {
  ...
  return (
    <div
      className="rounded-3xl border border-slate-200/70 bg-white/70 backdrop-blur-xl shadow-[0_20px_50px_-30px_rgba(15,23,42,0.4)] overflow-hidden flex flex-col"
      style={{ height: 560 }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <span className="relative flex h-2.5 w-2.5">
            ...
            <span
              className={`relative inline-flex rounded-full h-2.5 w-2.5 ${isActive ? "bg-emerald-500" : "bg-slate-300"}`}
            />
          </span>
          ...
          <>
            {messagesRealtime.map((msg) => {
              ...
              const text = msg.parts
                .map((p) =>
                  p.type === "text" ? p.content : p.type === "audio" ? p.transcript : "",
                )
                .filter(Boolean)
                .join(" ");
              if (!text) return null;
            ...
            {pendingUserTranscript && (
              <div className="flex gap-2 items-end flex-row-reverse">
                <div className="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-semibold bg-blue-100 text-blue-700">
                  You
                </div>
            ...
            {pendingAssistantTranscript && (
              <div className="flex gap-2 items-end">
                <div className="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-semibold bg-slate-100 text-slate-600">
                  AI
                </div>
