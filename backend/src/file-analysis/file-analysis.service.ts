...
export class FileAnalysisService {
  ...
  async analyzeFile(file: Express.Multer.File, request: any) {
    if (!file) {
    ...
    const fileDataPayload: {
      ...
      ? { data: file.path, mimeType: file.mimetype, isPath: true }
    ...
    const response = await this.geminiService.generateContent(prompt, {
      ...
      fileData: {
        ...fileDataPayload,
        isPath: fileDataPayload.isPath
          ? String(fileDataPayload.isPath)
          : undefined,
      },
      thinkingBudget: 4096,
    });
